...
AA<-read.table("diffgene140947.txt", header=T, sep="\t", check.names=F)
BB<-read.table("diffgene_name_140947.txt", header=F, sep="\t", check.names=F)
rownames(BB)=BB[,1]
row.names(AA)=AA[,1]
CC=AA[rownames(BB),] 
write.table(CC, file = "diffgene_vene_140947.txt", sep = "\t", quote = FALSE, row.names = FALSE)


inputdocument="diffgene_vene_140947.txt"         
...
write.table(BBTab,file="GSEA140947.txt",sep="\t",quote=F,row.names = F)
...
if(nrow(BBUp)>=termNum){
  ...
  pdf(file="Figure3A.pdf", width=8, height=6)
  print(gseaplot)
  dev.off()
}
...
outputfile2= "Figure3C.pdf"

pFilter=0.05     
qFilter=0.05     


biof =read.table("140947upgene_name.txt", header=T, sep="\t", check.names=F)
...
outputfile2= "Figure3E.pdf"

pFilter=0.05     
qFilter=0.05 
biof =read.table("140947downgene_name.txt", header=T, sep="\t", check.names=F)
...
rt <- read.table("140947upgene_name.txt", header = TRUE, sep = "\t", check.names = FALSE)
...
write.table(KEGG, file = "KEGG_140947up.txt", sep = "\t", quote = FALSE, row.names = FALSE)
showNum <- min(15, nrow(KEGG))
pdf("Figure3G.pdf", width = 9, height = 7)
barplot(kk, drop = TRUE, showCategory = showNum, label_format = 100, color = colorSel)
dev.off()

# downgene

rt <- read.table("140947downgene_name.txt", header = TRUE, sep = "\t", check.names = FALSE)
genes <- unique(as.vector(rt[, 1]))
entrezIDs <- mget(genes, org.Hs.egSYMBOL2EG, ifnotfound = NA)
rt <- data.frame(genes, entrezID = as.character(entrezIDs))
gene <- entrezIDs[entrezIDs != "NA"]
...
write.table(KEGG, file = "KEGG_140947down.txt", sep = "\t", quote = FALSE, row.names = FALSE)
showNum <- min(15, nrow(KEGG))
pdf("Figure3I.pdf", width = 9, height = 7)
barplot(kk, drop = TRUE, showCategory = showNum, label_format = 100, color = colorSel)
dev.off()
