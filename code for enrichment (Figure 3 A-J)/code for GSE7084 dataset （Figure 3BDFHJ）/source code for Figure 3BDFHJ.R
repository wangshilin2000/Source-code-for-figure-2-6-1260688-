...
AA1<-read.table("diffgene7084.txt", header=T, sep="\t", check.names=F)
BB1<-read.table("diffgene_name_7084.txt", header=F, sep="\t", check.names=F)
rownames(BB1)=BB1[,1]
row.names(AA1)=AA1[,1]
CC1=AA1[rownames(BB1),] 
write.table(CC1, file = "diffgene_vene_7084.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# choose the interested pathway from document "GSEA7084.txt" and save them as "GSEA7084_sellect.txt"
